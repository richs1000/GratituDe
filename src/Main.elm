...
type
    ...
    | LIPageMsg LI.MsgLI
      -- holds message from Enhance You page
    | EYPageMsg EY.MsgEY
        ...
        ( currentPage, mappedPageCmds ) =
            ...
            case model.route of
                ...
                Route.EnhanceYouRoute ->
                    let
                        -- use the init function for the Enhance You page
                        -- to get the enhanceYou.model
                        -- the commands generated by initializing the EY page
                        ( modelForEY, commandsFromEY ) =
                            EY.initEY
                    in
                    -- currentPage =
                    ( EnhanceYouPage modelForEY
                      -- mappedPageCmds =
                    , Cmd.map EYPageMsg commandsFromEY
                    )
    in
...
updateMain msg model =
    case ( msg, model.currentPage ) of
        ...
        ( EYPageMsg msgEY, EnhanceYouPage modelEY ) ->
            let
                -- use the Log In update function to get new model and command
                ( updatedEYModel, updatedEYCmd ) =
                    EY.updateEY msgEY modelEY
            in
            -- update current page to have the new model
            -- update the user in the current model based on the user who just logged in
            ( { model | currentPage = EnhanceYouPage updatedEYModel }
              -- update the current command to be the command from the LI init function
            , Cmd.map EYPageMsg updatedEYCmd
            )
